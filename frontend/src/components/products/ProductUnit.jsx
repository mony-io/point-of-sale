...
const ProductUnit = () => {
  ...
  async function addUnit() {
    try {
      if (unit.unit === '') {
        ...
        setSpin(true)
        const res = await axios.post("http://localhost:3001/product-units", unit);
        if (res.data.success) {
          setUnit({ unit: "" })
  ...
  const handleUpdate = async () => {
    ...
    try {
      if (unit.unit.trim() !== '') {
        const res = await axios.put(`http://localhost:3001/product-units/${unit.id}`, unit)
        if (res.data.success) {
          setUnit({ unit: "" })
          setColorStle("bg-green-100 text-green-700")
          setMsg(res.data.message);
  ...
  return (<>
    <div className="p-5 h-screen bg-gray-100 flex-1">
      ...
      <div className="flex justify-between mb-3">
        ...
        />
        {/* add unit model */}
        <div className="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="addUnit" tabIndex="-1" aria-labelledby="exampleModalLgLabel" aria-modal="true" role="dialog" onClick={(e) => {
            ...
            <div className="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
              ...
              <div
                ...
                {spin ? <button type="button"
                  ...
                  <div className="spinner-border animate-spin inline-block w-5 h-5 border-4 rounded-full text-blue-100" role="status"></div>
                </button>
                  : <button type="button"
                    className="inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out ml-1" onClick={addUnit}>
                    submit
                  </button>
                }
                {/* end of spin button */}
          ...
          <tbody>
            {
              units.map((item, index) => {
                return (
                  <tr
                    className="text-center bg-white border-b hover:bg-gray-100" key={index + 1}
                  >
                    ...
                    <td className="p-3 whitespace-nowrap">
                      <button className="mx-2 px-5 py-1.5 rounded-lg font-medium tracking-wider text-blue-700 bg-blue-200 hover:shadow" data-bs-toggle="modal" data-bs-target="#updateUnit" onClick={
                        async () => {
                          const res = await axios.get(`http://localhost:3001/product-units/${item.id}`);
                      ...
                      {/* edit model */}
                      <div className="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="updateUnit" tabIndex="-1" aria-labelledby="exampleModalLgLabel" aria-modal="true" role="dialog" onClick={(e) => {
                          ...
                          <div className="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                            ...
                            <div className="modal-body relative p-4 mt-5 mb-5 text-left">
                              ...
                              <input className="form-control
                                ...
                                ease-in-out
                                m-0
                              focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" placeholder="Unit" id="exampleFormControlInput1"
                                value={unit.unit}
                                onChange={handleChange}
                                name="unit"
                              />
                            ...
                            <div
                              ...
                              <button type="button"
                                className="inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out ml-1" onClick={handleUpdate}>update</button>
                            </div>
                          </div>
                        </div>
                      </div>
