...
const Category = () => {
  ...
  const createCategory = async () => {
    try {
      if (category.categoryName.trim() !== "") {
        const res = await axios.post("http://localhost:3001/categories", category);
        // console.log(res)
        if (res.data.success) {
          clearData();
  ...
  const handleUpdate = async () => {
    ...
    try {
      if (category.categoryName.trim() !== '') {
        const res = await axios.put(`http://localhost:3001/categories/${category.id}`, category)
        if (res.data.success) {
          clearData();
          setColorStle("bg-green-100 text-green-700")
          setMsg(res.data.message);
    ...
    <>
      <div className="p-5 h-screen bg-gray-100 flex-1">
        ...
        <div className="flex justify-between mb-3">
          <button className="hidden md:block ml-1 px-4 py-1.5 rounded-lg font-medium tracking-wider bg-teal-400 text-neutral-900 hover:text-white hover:shadow" data-bs-toggle="modal" data-bs-target="#addCategory">
            បន្ថែម
          </button>
          <input
          ...
          />
          {/* add category model */}
          <div className="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="addCategory" tabIndex="-1" aria-labelledby="exampleModalLgLabel" aria-modal="true" role="dialog" onClick={(e) => {
              ...
              <div className="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                ...
                <div
                  ...
                  <button type="button"
                    ...
                    data-bs-dismiss="modal" onClick={clearData}>បិទ</button>

                  {/* spin button */}

                  <button type="button"
                    className="inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out ml-1" onClick={createCategory}>
                    យល់ព្រម
                  </button>
            ...
            <tbody>

              {
                categories.map((item, index) => {
                  return (
                    <tr
                      className="text-center bg-white border-b-2 border-gray-100" key={index + 1}
                    >
                      ...
                      <td className="p-3 whitespace-nowrap">
                        <button className="mx-2 px-5 py-1.5 rounded-lg font-medium tracking-wider text-blue-700 bg-blue-200 hover:shadow" data-bs-toggle="modal" data-bs-target="#updateCategory" onClick={
                          async () => {
                            try {
                              const res = await axios.get(`http://localhost:3001/categories/${item.id}`);
                        ...
                        {/* update brand model */}
                        <div className="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="updateCategory" tabIndex="-1" aria-labelledby="exampleModalLgLabel" aria-modal="true" role="dialog" >
                          <div className="modal-dialog modal-lg relative w-auto pointer-events-none">
                            <div className="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                              <div className="modal-header flex flex-shrink-0 items-center justify-between p-4 border-b border-gray-200 rounded-t-md">
                              ...
                              <div className="modal-body relative p-4 mt-5 mb-5 text-left">
                                <label htmlFor="unit" className="form-label inline-block mb-2 text-gray-700">ប្រភេទផលិតផល</label>
                                <input className="form-control
                                  block
                                  w-full
                                  px-4
